...
void COXGridList::SetEditable(BOOL bEdit /* = TRUE */, int nColumn /* = -1 */, int EditType/* = EDIT_TYPE_STRING*/)
{
	...
	if (nColumn != -1)
	{
		...
		m_rgbEditType.SetAt(nColumn, (WORD)EditType);
	}
	else
	{
		for (int nIndex = 0; nIndex < m_rgbEditable.GetSize(); nIndex++)
		{
			...
			m_rgbEditType.SetAt(nIndex, (WORD)EditType);
		}
	}
}
...
afx_msg LRESULT COXGridList::OnInsertColumn(WPARAM wParam, LPARAM lParam)
{
	...
	if (dwIndex != -1)
	{
		...
		m_rgbEditType.InsertAt(dwIndex, (WORD)EDIT_TYPE_STRING);
