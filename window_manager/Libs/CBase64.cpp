...
char* CBase64::GetAnsiString(const CString &s) 
{ 
...
unsigned int CBase64::EncodeString (CString InputString, CString& OutputString)
	{
	char *buf_in, *buf_out;
	unsigned int len_in, len_out;

	buf_in = GetAnsiString(InputString);
	len_in = strlen(buf_in);

	len_out = CreateMatchingEncodingBuffer(len_in, &buf_out);
	EncodeBuffer(buf_in, len_in, buf_out);
	
	OutputString = buf_out;
...
unsigned int CBase64::DecodeString (CString InputString, CString& OutputString)
	{
	char *buf_in, *buf_out;
	unsigned int len_in, len_out;
	...
	len_out = CreateMatchingDecodingBuffer(buf_in, &buf_out);
	DecodeBuffer(buf_in, buf_out);
	buf_out = (char *)realloc(buf_out, len_out+1); buf_out[len_out] = 0;
